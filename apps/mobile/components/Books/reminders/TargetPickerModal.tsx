...
export function TargetPickerModal({
  ...
  // (optional) hydrate when modal opens
  React.useEffect(() => {
    if (visible && !hydrated) hydrate();
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((t) => t.title.toLowerCase().includes(query));
  }, [rows, q]);

  const renderItem = ({ item }: { item: TargetRow }) => {
